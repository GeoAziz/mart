...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const toggleWishlistItem = async (product: ProductForWishlist) => {
    ...
    try {
      const token = await currentUser.getIdToken();
      if (isInWishlist) {
        await fetch(`/api/users/me/wishlist/${product.id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
        ...
        const newItem: WishlistItemApiType = await response.json();
        setWishlistItems(prev => [...prev, {...newItem, addedAt: new Date(newItem.addedAt)}]);
