...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Robustly convert Firestore Timestamp to a client-safe Date object
    ...
    if (updatedAtRaw instanceof Timestamp) {
        ...
        // Handle cases where it might be a string, number, or other format
        const parsedDate = new Date(updatedAtRaw as any);
        updatedAtClient = !isNaN(parsedDate.getTime()) ? parsedDate : new Date();
    } else {
        // Fallback if the updatedAt field doesn't exist
