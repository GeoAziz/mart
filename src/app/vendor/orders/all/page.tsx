...
import { Eye, MoreHorizontal, ShoppingBag, PackageCheck, PackageX, Filter, Edit, Send, PackageSearch, Loader2 } from 'lucide-react';
...
const getStatusIcon = (status: string) => {
    switch (status.toLowerCase()) {
      ...
      case 'processing':
        return <Edit className="h-4 w-4 text-yellow-400" />; // Using Edit for processing
      case 'pending':
  ...
  if (isLoading && orders.length === 0) {
    return (
      <div className="flex justify-center items-center h-64">
        ...
        <p className="ml-4 text-xl text-muted-foreground">Loading orders...</p>
      </div>
    );
  }
        ...
        <CardHeader className="flex flex-row items-center justify-between">
            <div>
                <CardTitle className="text-2xl font-headline text-glow-primary flex items-center">
                    <ShoppingBag className="mr-3 h-6 w-6 text-primary" /> All Your Orders
                </CardTitle>
        ...
        <CardContent>
          {orders.length > 0 ? (
                        ...
                        <DropdownMenuContent align="end" className="bg-card border-primary shadow-lg">
                          ...
                          {order.status === 'processing' && (
                           ...
                           {(order.status === 'pending' || order.status === 'processing') && (
                            <DropdownMenuItem onClick={() => handleUpdateStatus(order.id!, 'cancelled')} className="text-destructive hover:bg-destructive/10 hover:!text-destructive focus:bg-destructive/20 focus:!text-destructive cursor-pointer">
                              <PackageX className="mr-2 h-4 w-4" /> Cancel This Order
                            </DropdownMenuItem>
                          )}
                        </DropdownMenuContent>
                      </DropdownMenu>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm text-muted-foreground">You haven't received any orders yet, or your filters returned no results.</p>
            </div>
          )}
        </CardContent>
      </Card>
